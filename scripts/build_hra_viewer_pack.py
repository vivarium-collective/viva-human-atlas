...
Resolves a Female kidney reference-organ GLB URL from the live HRA CCF API
(`hra_api.fetch_reference_organs`), builds model coverage (Task B,
`coverage.build_coverage`) and spatial links (Task C,
`spatial_link.build_spatial_links`) against the live crosswalk + BioModels
search, and writes the resulting self-contained three.js viewer pack
(``coverage.json``, ``spatial-links.json``, ``config.json``, ``index.html``,
``viewer.js``) under ``studies/model-coverage-3d/viz/hra/`` via
...
def resolve_kidney_female_glb_url() -> str:
    """Pick a Female kidney reference-organ GLB URL from the live HRA API.

    Selects the reference-organ entry whose source `@id` contains "kidney"
    ...
    for entry in organs:
        ...
        if (
            "kidney" in ref_organ_id.lower()
            and entry.get("sex") == "Female"
            and asset_url.endswith(".glb")
        ):
            return asset_url
    raise RuntimeError(
        "No Female kidney reference organ with a .glb asset_url found in "
        "fetch_reference_organs()"
    )


def main() -> None:
    print("Resolving kidney (female) reference-organ GLB URL...")
    kidney_glb_url = resolve_kidney_female_glb_url()
    print(f"  -> {kidney_glb_url}")
