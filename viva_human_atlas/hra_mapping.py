...
def map_to_hra(uberon_ids, name: str, organ_index: dict, *, ftus: Optional[list] = None) -> dict:
    ...
    come from the model `name` via `biomodel_do._match_organ_key` (the same
    organ-synonym seam `annotate_biomodel` uses). FTUs are the curated FTUs
    belonging to a matched organ, and `cell_types` are those FTUs' cell types
    (deduped by CL, first-seen order).
