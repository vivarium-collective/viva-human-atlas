...
def test_run_glucose_regulation_shape(monkeypatch):
    # Mock search so this runs one known glucose model deterministically offline.
    import viva_human_atlas.composites.glucose_regulation as gr
    ...
    entry = report["BIOMD0000000372"]
    assert set(entry) == {"engines", "comparison", "error"}
...
- [ ] **Step 6: Run the offline tests, expect PASS** (this actually runs COPASI + Tellurium on `BIOMD0000000372` — if that ID fails to load in one engine, the entry's `error` is populated and the test still passes on shape; confirm at least the shape asserts hold):

```bash
cd ~/code/viva-human-atlas && .venv/bin/python -m pytest tests/test_end_to_end.py -m "not network" -v
```

- [ ] **Step 7: Run the live test once to confirm the real search works** (network required; not part of default CI):

```bash
cd ~/code/viva-human-atlas && .venv/bin/python -m pytest tests/test_end_to_end.py -m network -v
```

If `BIOMD0000000372` does not appear in the live results or fails to load, pick a glucose-regulation ID that does appear (from the live search output) and update the mock in Step 4 accordingly.
